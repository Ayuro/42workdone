...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2017/01/11 15:07:36 by flemaist          #+#    #+#             */
/*   Updated: 2017/01/11 15:07:36 by flemaist         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	check_do_done_helper(char **arg)
{
	...
	{
		if (arg[0] && !arg[1])
			ft_putstr_fd("Joshua is confused: missing `do'\n", 2);
		else if (arg[0] && arg[1])
			ft_putstr_fd("Joshua is confused: missing `done'\n", 2);
		else
			ft_putstr_fd("Joshua is confused: missing `do' and `done'\n", 2);
		return (0);
	}
	return (1);
}
...
int			check_do_done(char **arg)
{
	...
	{
		ft_putstr_fd("Joshua is confused: parse error near `done'\n", 2);
		return (0);
	}
...
int			is_script(char **arg)
{
	...
	if (!arg || !arg[0])
	{
		ft_putstr_fd("Joshua is confused: missing command\n", 2);
		return (0);
	}
