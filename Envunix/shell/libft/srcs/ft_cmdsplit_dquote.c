...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   ft_cmdsplit_dquote.c                               :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: flemaist <marvin@42.fr>                    +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2017/01/11 13:18:24 by flemaist          #+#    #+#             */
/*   Updated: 2017/01/11 13:18:25 by flemaist         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int		get_next_quote(char const *s, int i)
{
	...
	save = i;
	while (s[i] && (s[i] != 34 || (s[i] == 34 &&
					ft_backslash_check(s, i))))
		i++;
	if (s[i] == 34 && !ft_backslash_check(s, i))
		return (i);
	else
		return (save - 1);
}
...
static int		nbhelp(char const *s, int j, int *i_quote)
{
	...
	i = 0;
	if (s[i] == 34 && !ft_backslash_check(s, i) && i > *i_quote)
	{
		*i_quote = i;
		return (i + 1);
	}
...
static char		*nblet(char const *s, const int j, int *i_quote)
{
	...
	o = i;
	if ((i - 1) > 0 && s[i - 1] == 34 && !ft_backslash_check(s, i - 1))
	{
		k = i;
		i = get_next_quote(s, i);
		k = i - k;
	}
	while (!ft_isspace(s[i]) && s[i] && (s[i] != 34 || (s[i] == 34 &&
					ft_backslash_check(s, i))))
	{
		k++;
		i++;
...
char			**ft_cmdsplit_dquote(char const *s)
{
	int			i;
	char		**tab;
	int			j;
